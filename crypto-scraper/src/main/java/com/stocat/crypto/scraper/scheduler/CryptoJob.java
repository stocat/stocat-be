package com.stocat.crypto.scraper.scheduler;

import com.stocat.crypto.scraper.config.UpbitApiProperties;
import com.stocat.crypto.scraper.dto.MarketInfo;
import com.stocat.crypto.scraper.service.SubscriptionCodeService;
import com.stocat.crypto.scraper.service.UpbitCryptoMarketService;
...
//    @Scheduled(fixedRate = 60_000, initialDelay = 1000) // 로컬 테스트용
    public void refreshHodCodeAndAddJobSubscribe() {
        Set<MarketInfo> dailyCodes = upbitCryptoMarketService.getDailyCrypto(upbitApiProperties.getTopLimit());
        subscriptionCodeService.refreshHotAndSubscribeCodes(dailyCodes.stream().map(MarketInfo::code).toList().toArray(new String[0]));
